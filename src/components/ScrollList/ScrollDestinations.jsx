...
function ScrollDestinations({ destinations, isFetching }) {
  ...
  const renderItem = (destination) => (
    <Link to={"/destination/" + destination.name.toString()} key={destination.name.toString()} style={{ textDecoration: 'none', color: 'inherit' }}>
      <ListItem divider>
          <Grid container alignItems="center" justifyContent="center">
          <Grid item>
              <img
                alt="country flag"
                src={require("assets/img/flags/" + destination.countryCode + ".png")}
