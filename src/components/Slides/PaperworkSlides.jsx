...
import { Box, useTheme, Paper, useMediaQuery, Typography, Stack } from "@mui/material";
...
const PaperworkSlides = () => {
  ...
  const createSlides = () => {
    let slides = [];
    const totalItemsNeeded = Math.ceil(paperworkServices.length / slice) * slice;
    for (let i = 0; i < totalItemsNeeded; i += slice) {
      let itemsForCurrentSlide = [];
      for (let j = 0; j < slice; j++) {
        const currentItemIndex = (i + j) % paperworkServices.length; // Use modulo to loop back to the start of the array
      ...
      slides.push(
        ...
        >
          ...
          >
            <Paper sx={{height: "31vh", 
              cursor: "pointer",}} 
            onClick={() => handleServiceClick(service.path)}>
              <img src={`${imagesCdn}/icons/${service.image}`} alt={service.name} style={{height: "100%"}}/>
            </Paper>
            <Typography
              variant="h4"
              sx={{
