...
export const onAuthStateChanged = (onAuthCallback) => api.onAuthStateChangedCallback(onAuthCallback)
...
export const storeAuthUser = (authUser) => (dispatch, getState) => {
  if (authUser) {
    api
      ...
      .then(
        user => dispatch({ type: SET_AUTH_USER, user: user, isAuthResolved: true }),
